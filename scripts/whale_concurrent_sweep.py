"""Sweep max_concurrent positions — what's the impact on compounding?

Tests: 2, 3, 4, 5 concurrent slots
Also tests dynamic: start with 2, scale up at balance thresholds
12-month projection, no kill switch, $100 start
"""
...
def get_max_concurrent(bankroll: float, mode: str) -> int:
    """Get max concurrent based on mode.

    Static modes: "2", "3", "4", "5"
    Dynamic mode: "dynamic" — scales with balance
    """
    if mode == "dynamic":
        if bankroll >= 10_000: return 5
        if bankroll >= 5_000: return 4
        if bankroll >= 1_000: return 3
        return 2
    return int(mode)


def simulate(daily_signals, *, starting_bankroll=100.0, concurrent_mode="2"):
    ...
    for day_idx, day_sigs in enumerate(daily_signals):
        ...
        max_c = get_max_concurrent(bankroll + sum(p.cost for p in open_pos), concurrent_mode)

        for sig in day_sigs:
            if stopped or bankroll <= 0: break
            if skip_next: skip_next = False; continue
            if sig["mid"] in traded: continue
            if len(open_pos) >= max_c: continue
...
async def main():
    ...
    while True:
        ...
        if len(all_trades) % 20000 < 1000: print(f"  {len(all_trades)} trades...", flush=True)
    ...
    while True:
        ...
        if len(all_whale) % 100000 < 1000: print(f"  {len(all_whale)} whale trades...", flush=True)
    print(f"  {len(all_whale)} settlement trades")
    ...
    n_days = len(set(s.signal_time.strftime("%Y-%m-%d") for s in sigs))
    wins = sum(1 for s in sigs if s.won)
    print(f"\n  {len(sigs)} signals, {n_days} days, {wins/len(sigs)*100:.1f}% WR, {len(sigs)/n_days:.1f}/day")
    ...
    TARGET_DAYS = 365
    N_SIMS = 500
    modes = ["2", "3", "4", "5", "dynamic"]

    print(f"\n{'='*90}")
    print(f"  CONCURRENT POSITION SWEEP — 12 months, no kill switch, $100 start")
    print(f"  {N_SIMS} Monte Carlo sims per mode")
    print(f"{'='*90}")

    for mode in modes:
        if mode == "dynamic":
            label = "DYNAMIC (2→3@$1K→4@$5K→5@$10K)"
        else:
            label = f"MAX_CONCURRENT = {mode}"
        ...
        for i in range(N_SIMS):
            ...
            r = simulate(tiled, concurrent_mode=mode)
        ...
        print(f"\n  {label}")
        print(f"    12mo: median ${np.median(finals):>12,.0f} | p5 ${np.percentile(finals, 5):>12,.0f} | p95 ${np.percentile(finals, 95):>12,.0f}")
        print(f"    Trades: {np.mean(mc_trades):.0f} avg")

        # Compact monthly curve
        curve = []
        for m in range(12):
            if mc_monthly[m]:
                arr = np.array(mc_monthly[m])
                curve.append(f"M{m+1}=${np.median(arr):,.0f}")
        print(f"    Curve: {' → '.join(curve)}")

        # Milestones
        milestones = [1_000, 10_000, 100_000, 500_000, 1_000_000]
        for target in milestones:
            ...
            if reached > 0:
                ...
                print(f"    ${target:>10,}: {pct:>5.1f}% | {med_d:>3}d median")
            else:
                print(f"    ${target:>10,}:   0.0%")
